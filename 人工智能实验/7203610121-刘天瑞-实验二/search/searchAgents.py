...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.startingPosition, tuple()#使用元组这一类结构来确认已经访问过的4个角落的坐标状态
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return len(state[1]) == 4#如果4个角落都已经被访问过，那么返回1，否则就返回0
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            next_state = self.getNextState(state, action)#使用getNextState函数来获得子节点位置
            next_state_cost = self.getActionCost(state, action, next_state)#使用getActionCost函数来获得每一步所耗的费用
            children.append((next_state, action, next_state_cost))#按照之前所查看的函数，同样返回3个状态值：子节点，遍历动作方向，每一步所耗的费用
    ...
    def getNextState(self, state, action):
        ...
        nextx, nexty = int(x + dx), int(y + dy)
        "*** YOUR CODE HERE ***"
        next_place = (nextx, nexty) 
        corners = list(state[1])#将元组结构强制转换成list表，并且对其中的元素进行更改
        if (next_place in self.corners) and (next_place not in corners):#如果下一个节点是角落则添加进元组内
            corners.append(next_place)
        return next_place, tuple(corners)
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if corner not in visited_corners:#如果有角落未被访问到
            #count += 1
            max_distance = max(max_distance, util.manhattanDistance(current_position, corner))#那么选用最大的曼哈顿距离作为启发式函数值
    #if count:
    #    return max_distance * 1.0 / count
    #else:
    #    return 0
    return max_distance
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    food_list = foodGrid.asList()#强制转换成不能被修改的一类集合
    max_distance = 0
    for food_postion in food_list:
        distance = mazeDistance(position,food_postion,problem.startingGameState)#选取最大的mazedistance作为启发式函数的计算值
        if distance > max_distance:
            max_distance = distance#取最大的距离
    return max_distance
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        return search.bfs(problem)#使用广度优先搜索（bfs）来依次去寻找到目标状态，即最近的food
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        return (x,y) in self.food.asList()#返回所有food的位置
        #util.raiseNotDefined()
